...
function SearchResults() {
  ...
  useEffect(() => {
    fetchHotels(searchCity || "all", sortBy || "name", order || "asc"); // Default: sorted by name
  }, [searchCity, sortBy, order]);

  // Fetch hotels based on city, default sorting by name
  const fetchHotels = async (city, sortBy, order) => {
    try {
      const queryParams = { sortBy, order };
  ...
  // Handle sorting by price when button is clicked
  const handleSortByPrice = () => {
    params.set("sortBy", "cheapestPrice");
    params.set("order", "asc");
    navigate(`?${params.toString()}`);
    fetchHotels(searchCity || "all", "cheapestPrice", "asc");
  };
